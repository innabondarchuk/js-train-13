...
function sortUrlParams(url) {
  ...
  const urlObj = new URL(url);
  const paramsArr = Array.from(urlObj.searchParams.entries());
  paramsArr.sort((a, b) => a[0].localeCompare(b[0]));
  urlObj.search = '';
...
function getURLValues(url) {
  ...
  const urlObj = new URL(url);
  const URLSearchParams = urlObj.searchParams;
  const paramKeys = Array.from(URLSearchParams.keys());
  const parmValues = [];
  for (const key of paramKeys) {
    const value = URLSearchParams.getAll(key);
    parmValues.push(...value)
  };
  return parmValues;
}
